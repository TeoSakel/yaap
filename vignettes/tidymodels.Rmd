...
Here we fit an `archetypes_path` for `K = 1, ..., 6` to the normalized
arrest variables. The path stores one copy of the common data and lets us
extract any fitted model with `[[` after scoring the sequence.
...
rss_tbl <- screeplot(fit_path, y = "loss", plot = FALSE)
r2_tbl <- screeplot(fit_path, y = "r2", plot = FALSE)
aic_tbl <- screeplot(fit_path, plot = FALSE)
